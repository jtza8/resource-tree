...
(def-test-method test-setf-node-of ((test resource-tree-test))
  (let ((tree (make-hash-table))
    ...
    (setf (node-of tree '(:a)) 5)
    (assert-equal 5 (node-of tree :a))
    (setf (node-of tree var) 6)
    (assert-equal 6 (node-of tree :a))
    (setf (node-of tree (list (car var))) 7)
    (assert-equal 7 (node-of tree :a))))
